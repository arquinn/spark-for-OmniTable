 ...
 */
trait PassThrough extends UnaryNode {
  val produced: Seq[Attribute]
  override def producedAttributes : AttributeSet = AttributeSet(produced)
  override def output : Seq[Attribute] = produced ++ child.output


  // because it isn't a case class, we need a special copy operator here?
