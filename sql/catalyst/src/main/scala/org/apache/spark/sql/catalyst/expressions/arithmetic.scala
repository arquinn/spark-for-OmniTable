  ...
  examples = """
    Examples:
      > SELECT _FUNC_(10, 9, 2, 4, 3);
       2
  """)
case class SelectOnMin(selectionChildren: Seq[Expression],
                          children: Seq[Expression]) extends ComplexTypeMergingExpression {
  ...
  override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
    val evalChildren = children.map(_.genCode(ctx))
    val evalSelectors = selectionChildren.map(_.genCode(ctx))

    ev.isNull = JavaCode.isNullGlobal(ctx.addMutableState(CodeGenerator.JAVA_BOOLEAN, ev.isNull))
    val evals = (evalChildren zip evalSelectors).map(t => t match {
      case (eval, seval) =>
        s"""
           |${seval.code}
           |${eval.code}
           |comb = [${seval}, ${eval}]
           |${ctx.reassignIfSmaller(dataType, ev, "comb")}
           |
        """.  stripMargin
    })
