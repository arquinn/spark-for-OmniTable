 ...
 * value).
 */
object ExtractEquiOrderedJoinKeys extends Logging with PredicateHelper {
  /** (joinType, leftKeys, rightKeys, condition, leftChild, rightChild) */
  type ReturnType =
    (Seq[Expression], Seq[Expression], Seq[Expression], Seq[Expression],
      Option[Expression], LogicalPlan, LogicalPlan, OrderedJoinType)
  type subReturnType = (Seq[Expression], Seq[Expression], Seq[Expression], Seq[Expression],
    Option[Expression], LogicalPlan, LogicalPlan)

  def unapply(plan: LogicalPlan): Option[ReturnType] = plan match {

    case _ @ SDOrderedJoin(left, right, lOexpr, rOexpr, condition, t) =>
      getInfo(left, right, lOexpr, rOexpr, condition).map(rt =>
        (rt._1, rt._2, rt._3, rt._4, rt._5, rt._6, rt._7, t))

    case _ => None
  }

  def getInfo(left: LogicalPlan, right: LogicalPlan, lOexprs: Seq[Expression],
             rOexprs: Seq[Expression], condition: Option[Expression]): Option[subReturnType] = {
    logDebug(s"Considering join on: $condition")
    // Find equi-join predicates that can be evaluated before the join,  thus can be used
    // as join keys.
    val predicates = condition.map(splitConjunctivePredicates).getOrElse(Nil)
    ...
    val oPreds = predicates.filterNot {
      case EqualTo(l, r) if l.references.isEmpty || r.references.isEmpty => false
      case EqualTo(l, r) =>
        canEvaluate(l, left) && canEvaluate(r, right) ||
          canEvaluate(l, right) && canEvaluate(r, left)
    ...
    if (joinKeys.nonEmpty) {
      ...
      Some((lKeys, rKeys, lOexprs, rOexprs,
        oPreds.reduceOption(And), left, right))
    } else {
      logDebug(s"no JoinKeys!?")
      Some((Nil, Nil, lOexprs, rOexprs,
        oPreds.reduceOption(And), left, right))
    }
  }
}
